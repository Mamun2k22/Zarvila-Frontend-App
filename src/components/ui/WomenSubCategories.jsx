...
// const VISIBLE_SUB_TILES = 6; // একসাথে ৬টা টাইল
...
//           `${import.meta.env.VITE_APP_SERVER_URL}api/products/public/home/women?all=true`
//         );
//         const data = await res.json();
//         const list = Array.isArray(data) ? data : [];
//         setAllProducts(list);
//         setProducts(list); // শুরুতে সব দেখাবে
//       } catch (err) {
...
//         const womenSubs = (Array.isArray(data) ? data : []).filter(
//           (sub) => sub.parentCategory?.name === "Women"
//         );

//         setSubTiles(womenSubs);
//       } catch (err) {
//         console.error("Error loading women subcategories:", err);
//         setSubTiles([]);
//       } finally {
...
//             <h2 className="text-xl font-bold tracking-wide text-gray-900 uppercase md:text-start text-center">
//               Women Shop
//             </h2>
//           </div>
...
//       {/* নিচে WOMEN products (filtered / all) */}
...
//             </div>
//           ) : (
//             <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 mx-4 md:mx-0">
...
// export default WomenSubCategories;
// উপরের কোড আপলোড সার্ভারের জন্য কমেন্ট করা
import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
...
const withBase = (path) => {
  ...
  // ImgBB/full URL হলে direct return
  ...
  useEffect(() => {
    const fetchWomenSubs = async () => {
      try {
        ...
        const womenSubs = (Array.isArray(data) ? data : []).filter(
          (sub) => sub.parentCategory?.name?.trim().toLowerCase() === "women"
        );
          ...
          <div className="relative w-full md:w-4/5">
            {hasPrev && !subsLoading && (
              <button
                type="button"
                onClick={handlePrev}
                className="hidden md:flex items-center justify-center absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 h-10 w-10 rounded-full bg-white shadow-md border border-gray-200 cursor-pointer hover:bg-gray-50 hover:shadow-lg z-10"
              >
                <ArrowLeftIcon />
              </button>
            )}
            ...
            {hasNext && !subsLoading && (
              <button
                type="button"
                onClick={handleNext}
                className="hidden md:flex items-center justify-center absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 h-10 w-10 rounded-full bg-white shadow-md border border-gray-200 cursor-pointer hover:bg-gray-50 hover:shadow-lg z-10"
              >
                <ArrowRightIcon />
              </button>
            )}
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-0 px-4 md:px-0">
              {subsLoading ? (
                ...
                visibleSubs.map((sub) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <ImgIcon src={withBase(sub.image)} alt={sub.name} />
                      </div>
                ...
                <div className="col-span-3 md:col-span-6 text-center py-10 text-gray-500">
                  No women subcategories found.
                </div>
              )}
            </div>
          </div>
