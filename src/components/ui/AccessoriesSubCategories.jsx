...
//   // 👉 সব Accessories products
//   useEffect(() => {
//     const fetchAllAccessoriesProducts = async () => {
//       try {
//         const res = await fetch(
//           `${import.meta.env.VITE_APP_SERVER_URL}api/products/public/home/accessories?all=true`
//         );
...
//       } catch (err) {
//         console.error("Error loading accessories products:", err);
//         setAllProducts([]);
//         setProducts([]);
//       } finally {
//         setLoading(false);
...
//     fetchAllAccessoriesProducts();
//   }, []);

//   // 👉 Accessories subcategories
//   useEffect(() => {
//     const fetchAccessoriesSubs = async () => {
//       try {
//         const res = await fetch(
...
//         const accessoriesSubs = (Array.isArray(data) ? data : []).filter(
//           (sub) => sub.parentCategory?.name === "Accessories"
//         );

//         setSubTiles(accessoriesSubs);
//       } catch (err) {
//         console.error("Error loading accessories subcategories:", err);
//         setSubTiles([]);
//       } finally {
...
//   return (
//     <>
//       {/* উপরের ACCESSORIES subcategory strip */}
...
//       {/* নিচে Accessories products (filtered / all) */}
...
// export default AccessoriesSubCategories;
  ...
  useEffect(() => {
    const fetchAccessoriesSubs = async () => {
      try {
        ...
        const accessoriesSubs = (Array.isArray(data) ? data : []).filter(
          (sub) =>
            sub.parentCategory?.name?.trim().toLowerCase() === "accessories"
        );
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-0 px-4 md:px-0">
              {subsLoading ? (
                ...
                <div className="col-span-3 md:col-span-6 text-center py-10 text-gray-500">
                  No accessories subcategories found.
                </div>
              )}
            </div>
          </div>
