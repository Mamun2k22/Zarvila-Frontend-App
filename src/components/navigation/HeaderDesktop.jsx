...
//         <div className="w-full max-w-full mx-auto px-6 h-16 grid grid-cols-3 items-center">
//           {/* LEFT: social + promo */}
...
//       <nav className="w-full hidden md:block bg-[#000000] relative">
...
const HeaderDesktop = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setSiteLogo(settingsData.logoUrl || "");
        setSiteBrand(settingsData.brandName || "Zarvila");
  ...
  const socialLinks = useMemo(() => {
    if (!headerConfig?.socialLinks) return [];

    return Object.entries(headerConfig.socialLinks)
      .map(([key, value]) => {
        if (!value?.isActive || !value?.url) return null;
        const Icon = SOCIAL_ICON_MAP[key];
        if (!Icon) return null;
  ...
  return (
    <>
      <nav className="hidden md:flex bg-white border-b border-[#070707] font-manrope">
        <div className="w-full max-w-full mx-auto px-6 h-16 grid grid-cols-3 items-center">
          <div className="flex items-center gap-5 text-gray-500 min-w-0">
            ...
            {headerConfig?.showTopBar !== false && topBarItems.length > 0 && (
              <div className="topbar min-w-0 overflow-hidden">
                <div className="topbar-marquee ticker text-xs tracking-wide uppercase text-gray-600 font-medium truncate">
                  <span key={tickerIndex} className="topbar-text ppt-jump-in">
                    {topBarItems[tickerIndex]}
                  </span>
                </div>
